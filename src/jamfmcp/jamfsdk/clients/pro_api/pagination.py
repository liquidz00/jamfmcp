...
class Paginator:
    ...
    async def _request(self) -> Iterator[Page]:
        """
        Execute paginated requests and yield Page objects.
        ...
        if total_count > (results_count := len(first_page.results)):
            async for page in await self._api_client.concurrent_api_requests(
